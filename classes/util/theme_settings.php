...
class theme_settings {
    ...
    public function slideshow() {
        ...
        for ($i = 1; $i <= $slidercount; $i++) {
            ...
            $templatecontext['slides'][$j]['order'] = $theme->settings->$sliderorder;
            $templatecontext['slides'][$j]['key'] = $theme->settings->$sliderorder - 1;
            ...
            $templatecontext['slides'][$j]['url'] = $theme->settings->$sliderlink;
        ...
        $templatecontext['slides'][0]['active'] = true;
    ...
    public function news_items() {
        ...
        for ($i = 1; $i <= $newscount; $i++) {
            ...
            if (!empty($theme->settings->$newscontent)) {
                $templatecontext['news'][$j]['content'] = theme_abierta_get_setting($newscontent, 'format_html');
            }
        ...
        $templatecontext['newsinslider'] = $j > 4; // Only slider for 5 or more.
    ...
    public function logos_items() {
        ...
        for ($i = 1; $i <= $logoscount; $i++) {
            ...
            if (!empty($theme->settings->$logoscontent)) {
                $templatecontext['logos'][$j]['content'] = theme_abierta_get_setting($logoscontent, 'format_html');
            }
    ...
    public function stats_items() {
        ...
        for ($i = 1; $i <= $statscount; $i++) {
            ...
            if (!empty($theme->settings->$statscontent)) {
                $templatecontext['stats'][$j]['content'] = theme_abierta_get_setting($statscontent, 'format_html');
            }
    ...
    public function socialnet_items() {
        ...
        if (!empty($theme->settings->socialnet)) {
            ...
            foreach($links as $link) {
                ...
                $templatecontext['socialnet'][] = sprintf('<a href="%s" target="_blank" title="%s" %s><i class="fa %s"></i></a>', $uri, $title, $color, $icon);
            }
        }
    ...
    public function generalvars() {
        ...
        $templatecontext['wwwcurrenttheme'] = $CFG->wwwroot . '/theme/abierta';
