 ...
 * @return string
 */
function theme_abierta_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_abierta_set_bgsimg($theme);
 ...
 * @return string
 */
function theme_abierta_set_bgsimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_abierta_get_main_scss_content($theme) {
    global $CFG;

    $scss = file_get_contents($CFG->dirroot . '/theme/boost/scss/preset/default.scss');

    // Abierta scss.
    $abiertavariables = file_get_contents($CFG->dirroot . '/theme/abierta/scss/abierta/_variables.scss');
    $abierta = file_get_contents($CFG->dirroot . '/theme/abierta/scss/abierta.scss');

    // Combine them together.
    return $abiertavariables . "\n" . $scss . "\n" . $abierta;
}
 ...
 * @return string
 */
function theme_abierta_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_abierta_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('abierta');
 ...
 * @return string
 */
function theme_abierta_get_setting($setting, $format = false) {
    $theme = theme_config::load('abierta');
 ...
 */
function theme_abierta_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $CFG;
    ...
    if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
        ...
        foreach ($mycourseschildrennodeskeys as $k) {
            ...
            if ($CFG->navshowmycoursecategories) {
                ...
                $allchildrennodes = theme_abierta_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 * @return array
 */
function theme_abierta_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    foreach ($childrennodeskeys as $ck) {
        $allchildren = array_merge($allchildren, theme_abierta_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
    }
